...
function renderPizzas(pizzaList) {
  ...
  pizzaList.forEach((pizza) => {
    ...
    pizzaElement.innerHTML = `
            ...
            <div class='fff'>
                <p>от ${pizza.price} ₽</p>
                <button onclick="incrementCounter(this, ${pizza.id}, '${
      pizza.name
    }')">+ Добавить <span class="counter">${
      pizza.number > 0 ? pizza.number : ""
    }</span></button>
            </div>
        `;
...
function incrementCounter(button, pizzaId, pizzaName) {
  ...
  const pizzaCategory = Object.keys(obj).find((category) =>
    obj[category].some((pizza) => pizza.name === pizzaName)
  );
...
//корзина кнопка
  cartButton.innerHTML = `${totalPrice}₽ | <span><img src="./img/iconfinder_shopping-cart_2561279 1.svg" alt="#"></span>${totalAmount}`;


  localStorage.setItem(
    "cart",
    JSON.stringify(obj["All"].filter((pizza) => pizza.number > 0))
  );
}
